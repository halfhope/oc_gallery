...
class ControllerGalleryGalleries extends controller{
	...
	public function index() {
		if (isset($this->request->get['gallery_id'])) {
			...
			if (!empty($gallery_id)) {
				$this->response->redirect($this->url->link('gallery/gallery', 'gallery_id=' . (int)$gallery_id));
			} else {
		...
		foreach ($galleries as $key => $gallery) {
			...
			if ($config_gallery_show_counter[$this->current_store_id]) {
				...
				if (!empty($cached)) {
					...
					$cached = $this->model_gallery_gallery->getGalleryImages($gallery['gallery_id'], 1, 0);
