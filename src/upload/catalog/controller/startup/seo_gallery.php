 ...
 */
class ControllerStartupSeoGallery extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$this->cache_data) {
			$query            = $this->db->query("SELECT LOWER(`keyword`) as 'keyword', `query` FROM " . DB_PREFIX . "seo_url_gallery WHERE store_id = " . $this->current_store_id);
			...
			$query = $this->db->query("SELECT LOWER(`keyword`) as 'keyword', `query` FROM " . DB_PREFIX . "seo_url_gallery WHERE query LIKE '%gallery_id=%'");
			foreach ($query->rows as $row) {
				$this->cache_data['keywords'][$row['keyword']] = $row['query'];
				$this->cache_data['queries'][$row['query']]    = $row['keyword'];
			}
	...
	public function index() {
		...
		if (isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $part) {
				...
				if (count($rows) == sizeof($parts)) {
					...
					foreach ($parts as $part) {
						...
						if (isset($url[0]) && 'gallery_id' == $url[0]) {
							$this->request->get['gallery_id'] = $url[1];
							$this->gallery_flag             = 'gallery';
						} elseif (isset($url[0]) && 'gallery/galleries' == $url[0]) {
							$this->gallery_flag = 'galleries';
							$this->request->get['route'] = 'gallery/galleries';
						} elseif (isset($url[0]) && 'route' == $url[0]) {
			...
			if (isset($this->request->get['gallery_id'])) {
				$this->response->addHeader($this->request->server['SERVER_PROTOCOL'] . '/1.1 200 OK');
				$this->request->get['route'] = 'gallery/gallery';
			} elseif (isset($this->request->get['gallery/galleries'])) {
				$this->response->addHeader($this->request->server['SERVER_PROTOCOL'] . '/1.1 200 OK');
				$this->request->get['route'] = 'gallery/galleries';
			} else {
	...
	public function rewrite($link) {
		...
		if (isset($component['query'])) {
			...
			} elseif ('gallery/galleries' == $route) {

			} else {
				return $link;
			}

			if ('https' == $component['scheme']) {
	...
	private function validate() {
		if (empty($this->request->get['route']) || 'error/not_found' == $this->request->get['route']) {
			return;
		}
		if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) && strtolower($this->request->server['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			return;
		}
		if (isset($this->request->server['HTTPS']) && (('on' == $this->request->server['HTTPS']) || ('1' == $this->request->server['HTTPS']))) {
