...
class ControllerExtensionModuleGalleryGallery extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = [
			...
			'href'      => $this->url->link($this->_route . '/gallery', 'token=' . $this->session->data['token'], 'SSL'),
		];
		...
		if (!empty($data['galleries'])) {
			foreach ($data['galleries'] as $key => $value) {
				$value['edit_link'] = $this->url->link($this->_route . '/gallery/edit', 'token=' . $this->session->data['token'].'&gallery_id='.$value['gallery_id'], 'SSL');
		...
		// Links
		$data['add'] = $this->url->link($this->_route . '/gallery/add', 'token=' . $this->session->data['token'], 'SSL');
		$data['delete'] = $this->url->link($this->_route . '/gallery/delete', 'token=' . $this->session->data['token'], 'SSL');
		$data['copy'] = $this->url->link($this->_route . '/gallery/copy', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function getForm($data) {
		...
		if (!empty($gallery_query)) {
			$data['breadcrumbs'][] = [
				...
				'href'      => $this->url->link($this->_route . '/gallery/edit', 'gallery_id='.$this->request->get['gallery_id']. '&token=' . $this->session->data['token'], 'SSL'),
			];
		} else {
			$data['breadcrumbs'][] = [
				'text'      => $this->language->get('section_gallery'),
				'href'      => $this->url->link($this->_route . '/gallery/add', 'token=' . $this->session->data['token'], 'SSL'),
			];
		}
		...
		$data['save'] = $this->url->link($this->_route . '/gallery/add', 'token=' . $this->session->data['token'], 'SSL');

		$data['check_images'] = html_entity_decode($this->url->link($this->_route . '/gallery/checkImages', 'token=' . $this->session->data['token'], 'SSL'), ENT_QUOTES, 'UTF-8');
		$data['load_images'] = html_entity_decode($this->url->link($this->_route . '/gallery/loadImages', 'token=' . $this->session->data['token'], 'SSL'), ENT_QUOTES, 'UTF-8');

		if (isset($this->request->get['gallery_id'])) {
			$data['edit'] = $this->url->link($this->_route . '/gallery/edit', 'gallery_id='.$this->request->get['gallery_id']. '&token=' . $this->session->data['token'], 'SSL');
		}
