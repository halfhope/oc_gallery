...
class ModelGalleryGallery extends Model {
	...
	public function getGallery($gallery_id) {
		...
		$seo_query = $this->db->query("SELECT * FROM `". DB_PREFIX . "seo_url_gallery` WHERE `query` = 'gallery_id=" . $gallery['gallery_id'] . "' LIMIT 1");
	...
	public function editGallery($data) {
		...
		$this->db->query("UPDATE `" . DB_PREFIX ."gallery` SET `gallery_type` = '".$data['gallery_type']."', `enabled` = '".$data['enabled']."', `sort_order` = '".$data['sort_order']."', `last_modified` = '".date('Y-m-d H:i:s')."', `gallery_data` = '".$data['gallery_data']."' WHERE `gallery_id` = ".(int)$data['gallery_id']);

		$gallery_id = (int)$data['gallery_id'];

		$seo_query = $this->db->query("SELECT * FROM `". DB_PREFIX . "seo_url_gallery` WHERE `query` = 'gallery_id=".(int)$gallery_id."' LIMIT 1");

		if (empty($seo_query->row)) {
			if ($gallery_data['gallery_seo_url']) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url_gallery` (`query`, `keyword`) VALUES ('gallery_id=" . (int)$gallery_id . "', '" . $this->db->escape($gallery_data['gallery_seo_url']) . "')");
			}
		}else{
			if (empty($gallery_data['gallery_seo_url'])) {
				$this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url_gallery` WHERE `url_alias_id` = '".$seo_query->row['url_alias_id']."'");
			}else{
				$this->db->query("UPDATE `" . DB_PREFIX . "seo_url_gallery` SET `query` = 'gallery_id=" . (int)$gallery_id . "', keyword = '" . $this->db->escape($gallery_data['gallery_seo_url']) . "' WHERE `url_alias_id` = '".$seo_query->row['url_alias_id']."'");
			}
		}

		$store_query = $this->db->query("DELETE FROM `". DB_PREFIX . "gallery_to_store` WHERE `gallery_id` = " . (int)$gallery_id);
	...
	public function addGallery($data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX ."gallery`(`gallery_type`,`enabled`,`sort_order`,`last_modified`,`gallery_data`) VALUES ('".$data['gallery_type']."','".$data['enabled']."','".$data['sort_order']."','".date('Y-m-d H:i:s')."','".$data['gallery_data']."')");
	...
	public function deleteGallery($aids) {
		foreach ($aids as $key => $aid) {
			$this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url_gallery` WHERE `query` = 'gallery_id=".(int)$aid."'");
			$this->db->query("DELETE FROM `" . DB_PREFIX . "gallery` WHERE `gallery_id` = '" . (int)$aid . "'");
		}
	}
	
	public function updateGallerySeoUrl($data) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url_gallery` WHERE `query` = 'gallery/galleries'");

		foreach ($data['config_gallery_galleries_seo_name'] as $store_id => $alias) {
			if (!empty($data['config_gallery_galleries_seo_name'][$store_id])) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url_gallery` (`query`, `keyword`, `store_id`) VALUES ('gallery/galleries', '" . $this->db->escape($alias) . "', " . (int) $store_id . ")");
			}
		}
	}
