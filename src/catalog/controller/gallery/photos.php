...
class ControllerGalleryPhotos extends controller{
    ...
    public function index(){
        ...
        $album_cache_name = md5(json_encode($album_id.$this->current_language_id));
        $album = $this->cache->get('gallery_album_photos.'.$album_cache_name);
        if (empty($album) || (!$this->config->get('config_gallery_modules_cache_enabled'))) {
            ...
            $album['album_link'] = $this->url->link('gallery/photos', 'album_id='.$album['album_id']);
            
            $cached = $this->cache->get('album_photos.'.md5($album_id.'0'));
            if (!empty($cached)) {
                $album['images'] = $cached;  
            }else{
                $cached = $this->getAlbumImages($album_id, 0);
                $this->cache->set('album_photos.'.md5($album_id.'0'), $cached);
                $album['images'] = $cached;
            }
            ...
            $this->cache->set('gallery_album_photos.'.$album_cache_name, $album);
        }
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('gallery/photos', 'album_id='.$album_id, 'SSL'),          
            'separator' => $this->language->get('text_separator')
        );
